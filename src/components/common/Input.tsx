...
const Input:React.FC<Props> = ({label,type,placeholder,id,refr,onChange,className,value,name}) => {
    ...
    return <div className="h-[10vh] flex flex-col w-full">
        <label>{label}</label>
        <input type='text' ref={!name? refr:ref} value={value} name={name} onChange={!name? () => onChange(refr):() => onChange(ref)} placeholder={placeholder} id={id} className={className?.concat(' rounded-sm')}/>
        </div>
}
